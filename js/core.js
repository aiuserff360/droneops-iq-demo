...
VQ.ICONS = {
  ...
  drone: '<circle cx="5" cy="6" r="2.6"/><circle cx="19" cy="6" r="2.6"/><path d="M5 6l4 4.500M19 6l-4 4.500M8.500 10.500h7l1 3.500h-9zM9.500 14l-2 5M14.500 14l2 5M11 17h2"/>',
  drop: '<path d="M12 2.500s6.500 7 6.500 12a6.500 6.500 0 0 1-13 0c0-5 6.500-12 6.500-12z"/>',
  waves: '<path d="M2 7c2-2 4-2 6 0s4 2 6 0 4-2 6 0M2 12c2-2 4-2 6 0s4 2 6 0 4-2 6 0M2 17c2-2 4-2 6 0s4 2 6 0 4-2 6 0"/>',
  pump: '<rect x="3" y="9" width="11" height="9" rx="1.500"/><path d="M14 12h4a3 3 0 0 1 3 3v4M6 9V6h5v3M3 21h13M8.500 12v3M7 13.500h3"/>',
  tank: '<ellipse cx="12" cy="6" rx="8" ry="3"/><path d="M4 6v12c0 1.700 3.600 3 8 3s8-1.300 8-3V6M4 12c0 1.700 3.600 3 8 3s8-1.300 8-3"/>',
  pipe: '<path d="M2 8h6v8H2zM16 8h6v8h-6zM8 10h8M8 14h8M12 4v6M10 4h4"/>',
  factory: '<path d="M2 21V10l6 4v-4l6 4V4h4v17zM2 21h20M17 4V2M7 17h2M12 17h2"/>',
  thermo: '<path d="M14 14.800V4a2 2 0 0 0-4 0v10.800a4 4 0 1 0 4 0z"/><path d="M12 9v8"/>',
  fence: '<path d="M4 21V6l2-3 2 3v15M10 21V6l2-3 2 3v15M16 21V6l2-3 2 3v15M2 10h20M2 16h20"/>',
  valve: '<path d="M3 10l9 4.500L21 10v9l-9-4.500L3 19zM12 14.500V6M8 6h8"/>',
  flask: '<path d="M9 3h6M10 3v6.300L4.500 19a1.500 1.500 0 0 0 1.300 2h12.400a1.500 1.500 0 0 0 1.300-2L14 9.300V3M7 15h10"/>',
  battery: '<rect x="2" y="7" width="17" height="10" rx="2"/><path d="M22 11v2M6 10v4M9.500 10v4M13 10v4"/>',
  moon: '<path d="M21 12.800A9 9 0 1 1 11.200 3a7 7 0 0 0 9.800 9.800z"/>',
  swap: '<path d="M7 4 3 8l4 4M3 8h14M17 20l4-4-4-4M21 16H7"/>',
  helmet: '<path d="M3 17a9 9 0 0 1 18 0zM2 17h20v3H2zM12 8V4M9 9.500V5.500M15 9.500V5.500"/>',
...
VQ.fit = () => { const main = document.getElementById('main'); if (!main) return; const rail = document.body.classList.contains('ws-start') ? 0 : innerWidth <= 820 ? 60 : 204; const z = VQ.clamp((innerWidth - rail) / 1236, .55, 1); main.style.zoom = z === 1 ? '' : z.toFixed(3); VQ.zoom = z; };
...
VQ.step = d => { const i = VQ.J_INDEX[VQ.current] ?? (VQ.lastJ || {})[VQ.ws] ?? 0; const n = VQ.clamp(i + d, 0, VQ.JOURNEY.length - 1); VQ.go(VQ.JOURNEY[n].key); };

/* workspaces: each platform has its own sidebar, header text, hero and walkthrough */
VQ.RAIL = []; VQ.WS = {}; VQ.WSJ = {}; VQ.searchExtra = {};
VQ.workspace = (id, def) => { VQ.WS[id] = def; };
VQ.useWs = ws => { const W = VQ.WS[ws] || {}; VQ.ws = ws; VQ.RAIL = W.rail || []; const J = VQ.WSJ[ws] || { stages: [], journey: [], index: {} }; VQ.STAGES = J.stages; VQ.JOURNEY = J.journey; VQ.J_INDEX = J.index;
  document.body.className = 'ws-' + ws; const set = (id, html) => { const el = document.getElementById(id); if (el) el.innerHTML = html || ''; };
  set('brand-sub', W.brandSub); set('brand-motto', W.motto); set('brand-tag', W.tag); set('nation', W.claim); set('search-ph', W.search || 'Search…'); set('hero-txt', W.hero); set('date', W.date || 'Tue, 16 Sep 2026'); set('clock', W.time || '08:42 AM');
  const sw = document.getElementById('ws-switch'); if (sw) sw.innerHTML = ws === 'start' || window.VQ_ONLY ? '' : Object.entries(VQ.WS).filter(([k, w]) => w.rail).map(([k, w]) => `<button class="${k === ws ? 'on' : ''}" data-go="${w.home}" data-tip="${w.name}">${VQ.I(w.icon)}<span>${w.short}</span></button>`).join('') + `<button data-go="start" data-tip="Back to the start page">${VQ.I('grid')}</button>`; };
VQ.renderRail = active => { document.getElementById('rail').innerHTML = VQ.RAIL.map(r => `<button class="rail-item ${r[2] === active ? 'active' : ''}" data-go="${r[2]}">${VQ.I(r[0])}<span class="lbl">${r[1]}</span>${r[3] ? `<i class="rail-badge">${r[3]}</i>` : ''}</button>`).join(''); };

VQ.crumbs = list => !list || !list.length ? '' : `<div class="crumbs">${list.map((c, i) => (i ? VQ.I('chev') : '') + (c[1] ? `<a data-go="${c[1]}">${c[0]}</a>` : `<b>${c[0]}</b>`)).join('')}</div>`;
VQ.values = v => { const W = VQ.WS[VQ.ws] || {}; if (!W.values) return '<div class="view-pad"></div>'; const items = W.values.map((d, i) => ({ ...d, text: (v && v[i]) || d.text }));
  return `<div class="values">${items.map(d => `<div class="vi ${d.leaf ? 'leaf' : ''}">${VQ.I(d.icon)}<div><b>${d.title}</b><span>${d.text}</span></div></div>`).join('')}<div class="motto">${W.strip || ''}</div></div><div class="view-pad"></div>`; };

VQ.render = keepScroll => {
  const only = window.VQ_ONLY && VQ.WS[window.VQ_ONLY]; /* single-platform build: no start page, no switch */
  const r = VQ.parse(location.hash) || VQ.parse(only ? only.home : 'start'); const { S, sid, vid, param } = r; const V = S.views[vid];
  ...
  VQ.useWs(S.ws || 'start'); VQ._mounts = []; VQ.renderRail(S.rail || sid);
  document.getElementById('main').classList.toggle('dark', !!(V.dark ?? S.dark));
  ...
  if (VQ.J_INDEX[r.key] != null) { VQ.lastJ = VQ.lastJ || {}; VQ.lastJ[VQ.ws] = VQ.J_INDEX[r.key]; }
  VQ.runMounts(); VQ.renderDemo(); VQ.tip.hide();
  document.title = `${String(val(V.title || S.title)).replace(/<[^>]+>/g, '')} · VizionIQ`;
};

/* ---------- guided demo bar ---------- */
VQ.journey = (ws, stages) => { const journey = []; stages.forEach(st => st.screens.forEach(([key, label]) => journey.push({ key, label, stage: st.id }))); const index = {}; journey.forEach((j, i) => { if (index[j.key] == null) index[j.key] = i; }); VQ.WSJ[ws] = { stages, journey, index }; };
VQ.renderDemo = () => {
  const i = VQ.J_INDEX[VQ.current] ?? (VQ.lastJ || {})[VQ.ws] ?? 0; const j = VQ.JOURNEY[i]; const bar = document.getElementById('demo'); bar.style.display = j ? '' : 'none'; if (!j) return;
...
VQ.searchIndex = () => { const out = []; VQ.JOURNEY.forEach(j => out.push({ icon: 'file', label: j.label, kind: 'Screen', go: j.key })); (VQ.searchExtra[VQ.ws] || []).forEach(f => out.push(...f())); return out; };
VQ.openSearch = () => {
