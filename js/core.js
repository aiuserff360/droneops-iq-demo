...
VQ.useWs = ws => { const W = VQ.WS[ws] || {}; VQ.ws = ws; VQ.RAIL = W.rail || []; const J = VQ.WSJ[ws] || { stages: [], journey: [], index: {} }; VQ.STAGES = J.stages; VQ.JOURNEY = J.journey; VQ.J_INDEX = J.index;
  ...
  const sw = document.getElementById('ws-switch'); if (sw) sw.innerHTML = window.VQ_ONLY && VQ.projectSwitch ? VQ.projectSwitch() : ws === 'start' || window.VQ_ONLY ? '' : Object.entries(VQ.WS).filter(([k, w]) => w.rail).map(([k, w]) => `<button class="${k === ws ? 'on' : ''}" data-go="${w.home}" data-tip="${w.name}">${VQ.I(w.icon)}<span>${w.short}</span></button>`).join('') + `<button data-go="start" data-tip="Back to the start page">${VQ.I('grid')}</button>`; };
