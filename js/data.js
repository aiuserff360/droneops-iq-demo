...
VQ.ops = (() => {
  ...
  D.usecases = {
    dam: { key: 'dam', label: 'Dam inspection', id: 'M-2026-043', name: 'Site Inspection – North Ridge Dam', type: 'Inspection', usecase: 'Infrastructure Inspection', location: 'North Ridge Dam, Karnataka', pattern: 'Grid', areaLabel: 'Area', areaKm: 2.4, area: '2.4 km²', perimeter: '6.8 km',
    ...
    highway: { key: 'highway', label: 'Highway corridor patrol', id: 'M-2026-044', name: 'Corridor Patrol – NH-44 Km 78–92', type: 'Patrol', usecase: 'Highway Corridor Monitoring', location: 'NH-44, near Jadcherla, Telangana', pattern: 'Corridor', areaLabel: 'Corridor', areaKm: 1.7, corridorKm: 14, area: '14 km × 120 m', perimeter: '28.2 km',
    ...
    refinery: { key: 'refinery', label: 'Refinery thermal inspection', id: 'M-2026-045', name: 'Thermal Inspection – Process Unit 3', type: 'Inspection', usecase: 'Oil &amp; Gas Asset Inspection', location: 'Refinery, Process Unit 3 (CDU)', pattern: 'Orbits', areaLabel: 'Assets', areaKm: 0.3, assets: 6, area: '6 assets · 0.3 km²', perimeter: '2.1 km',
  ...
  /* the planner can edit any field; edits are stored per use case and the dependent numbers are recomputed */
  D.ucKey = () => VQ.store.get('uc', 'dam');
  D.uc = () => { const B = D.usecases[D.ucKey()] || D.usecases.dam, ov = VQ.store.get('ov_' + B.key, {}), U = { ...B, ...ov, drones: (ov.drones || B.drones).map(d => ({ ...d })) };
    const a = U.areaKm / B.areaKm, g = U.alt / B.alt, sp = U.speed / B.speed, o = ((100 - B.overlapF) * (100 - B.overlapS)) / Math.max(1, (100 - U.overlapF) * (100 - U.overlapS)), nd = Math.max(1, U.drones.filter(d => d.role !== 'Relay').length) / Math.max(1, B.drones.filter(d => d.role !== 'Relay').length);
    U.gsd = Math.round(B.gsd * g * 10) / 10; U.images = Math.round(B.images * a * o / (g * g) / 100) * 100; U.waypoints = Math.round(B.waypoints * a * Math.sqrt(o) / g); U.minutes = Math.max(5, Math.round(B.minutes * a * o / (sp * g) / nd)); U.dataGB = Math.max(5, Math.round(B.dataGB * U.images / B.images / 10) * 10);
    U.area = B.corridorKm ? `${Math.round(B.corridorKm * a)} km × 120 m` : B.assets ? `${Math.round(B.assets * a)} assets · ${U.areaKm} km²` : `${U.areaKm} km²`; if (ov.areaKm) U.perimeter = (parseFloat(B.perimeter) * Math.sqrt(a)).toFixed(1) + ' km';
    U.coverage = Math.min(99, Math.round(B.coverage + (o - 1) * 6 - (a - 1) * 2)); U.confidence = Math.min(98, Math.round(B.confidence + (o - 1) * 5 - (g - 1) * 6)); U.edited = Object.keys(ov).length; return U; };
  D.edit = (f, v) => { const k = D.ucKey(), ov = VQ.store.get('ov_' + k, {}); const n = parseFloat(v); ov[f] = f === 'areaKm' || f === 'alt' || f === 'speed' || f === 'overlapF' || f === 'overlapS' || f === 'interval' || f === 'spacing' || f === 'altSep' ? (isNaN(n) ? D.usecases[k][f] : n) : v; VQ.store.set('ov_' + k, ov); VQ.render(true); };
  D.editDrone = (i, f, v) => { const k = D.ucKey(), ov = VQ.store.get('ov_' + k, {}); const ds = (ov.drones || D.usecases[k].drones).map(d => ({ ...d })); if (f === 'remove') ds.splice(i, 1); else if (f === 'add') { const used = ds.map(d => d.id), pool = D.drones.filter(d => !used.includes(d.id) && d.status !== 'In Maintenance'); const d = pool[0] || { id: 'DR-0' + (20 + ds.length), model: 'DJI Matrice 30T' }; ds.push({ id: d.id, name: 'Drone ' + (ds.length + 1), model: d.model, role: 'Survey', tone: ['info', 'warn', 'violet', 'good', 'crit'][ds.length % 5], p1: 'RGB Camera', p2: 'Thermal Camera', batt: d.batt || 90, sector: 'Sector ' + 'ABCDEF'[ds.length] + ' · new split' }); } else ds[i][f] = v; ov.drones = ds; VQ.store.set('ov_' + k, ov); VQ.render(true); };
  D.resetEdits = () => { VQ.store.set('ov_' + D.ucKey(), {}); VQ.toast('Plan reset to its defaults'); VQ.render(true); };
